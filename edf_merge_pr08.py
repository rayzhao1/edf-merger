...
Usage (all absolute paths):
    python edf_merge_pr08.py ~/ray/ucsf/data_store2/presidio/nihon_kohden/PR08 ~/ray/ucsf/data_store2/presidio/nihon_kohden/PR08/catalogs/sub-PR08_edf-catalog_complete.csv out-PR08-9.16 --local
    submit_job -q pia-batch.q -c 8 -m 40 -o /userdata/rzhao/results-pr08.txt -n pr08 -x /userdata/rzhao/newenv/bin/python3 /userdata/rzhao/edf-merger/edf_merge_pr08.py
...
def get_first_date(csv_in: str, date_idx, sort=False) -> datetime:
    if sort:
        first_row = next(pd.read_csv(csv_in).sort_values(by='edf_name').itertuples(index=False))
    else:
...
def csv_parse_nights(
    ...
    start: datetime = get_first_date(csv_in, date_idx=start_idx, sort=sort)  # Set start date
    ...
    with open(csv_in) as csv_file:
        if sort:
            csv_reader = pd.read_csv(csv_in).sort_values(by='edf_name').itertuples(index=False)
        else:
...
def verify_night_ranges(patient, nights):
    ...
    def verify_night(
            night_num: int, 
            expected_range: range | tuple[range, tuple], 
            expected_len=None
        ):
        omit = tuple()
        if not isinstance(expected_range, range):
            expected_range, omit = expected_range
        correct_len = len(nights[night_num].intervals[0].files) == expected_len if expected_len else True
        correct_files = [Path(fn).name for fn in nights[night_num].intervals[0].files] == [f'{patient}_{i:04}.edf' for i in expected_range if f'{patient}_{i:04}.edf' not in omit]
        if not correct_len:
            for night in nights:
    ...
    def verify_pr08():
        """Total 9 nights. Missing data for night 2. PR08_0106 missing."""
        exp = (
            (range(41, 173), (106)),
            range(436, 568),
            range(715, 847),
    ...
    match patient:
        ...
        case 'PR07':
            return [
                ...
                '/data_store0/presidio/nihon_kohden/PR07/catalogs/sub-PR07_edf-catalog.csv',
                '/userdata/rzhao/out-PR07-5.7',
                ]
        case 'PR08':
            return [
                '/data_store2/presidio/nihon_kohden/PR08', 
                '/data_store2/presidio/nihon_kohden/PR08/catalogs/sub-PR08_edf-catalog_complete.csv',
                '/userdata/rzhao/out-PR08-9.21',
                ]
...
if __name__ == "__main__":
    ...
    PATIENT = 'PR08' # Must set if on server
    ...
    parser.add_argument("-s", "--sort", action="store_true",
                        help='flag to indicate that EDF catalog is unsorted by time')

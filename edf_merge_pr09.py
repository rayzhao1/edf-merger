...
Usage (all absolute paths):
    python edf_merge_pr09.py ~/ray/ucsf/data_store2/presidio/nihon_kohden/PR09 ~/ray/ucsf/data_store2/presidio/nihon_kohden/PR09/catalogs/sub-PR09_edf-catalog.csv out-PR00-10.9 --local
...
def verify_night_ranges(patient, nights):
    ...
    def verify_night(
        ...
        print(expected_range, omit)
        correct_len = len(nights[night_num].intervals[0].files) == expected_len if expected_len else True
        correct_files = [Path(fn).name for fn in nights[night_num].intervals[0].files] == [f'{patient}_{i:04}.edf' for i in expected_range if i not in omit]
        if not correct_len:
            for night in nights:
...
def get_server_run_inputs(patient):
    match patient:
        ...
        case 'PR09':
            return [
                '/data_store2/presidio/nihon_kohden/PR09', 
                '/data_store2/presidio/nihon_kohden/PR09/catalogs/sub-PR09_edf-catalog.csv',
                '/userdata/rzhao/out-PR09-10.9',
                ]
